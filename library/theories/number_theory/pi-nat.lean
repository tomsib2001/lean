...
-- (finset.induction_on
--    -- (λ pfn, (∏ p ∈ pfn, (if (pi p) then (p^(mult p n)) else 1)) > (0 : nat))
--   (prime_factors n)
--   (ProdEmpty_gt0 _)
--   (begin
...
--   -- Prodl_insert_of_not_mem
--   -- Prod
--   end)) 



-- (@finset.induction _ _ (λ pfn, (∏ p ∈ pfn, (if (pi p) then (p^(mult p n)) else 1)) > (0 : nat))
--   begin
...
-- list.cases_on (prime_factors n) (Prod_empty) (take x l, sorry)
  -- nat.cases_on (card (prime_factors n)) 
-- where is the product over a set even defined? -> in algebra/group_bigops.lean

-- variables (p : nat) (Hpprime : prime p)

-- -- definition is_p_nat := is_pi_nat (λ x, x = p)

-- definition decidable_p_nat_ext [instance] (n : nat) : 
--   decidable (is_pi_nat (λ x, x = p) n) := sorry

-- definition decidable_p_nat [instance] (n : nat) : decidable (is_pi_nat (λ x, x = p) n) := 
-- decidable_of_decidable_of_iff _ (is_pi_nat_ext_iff_is_pi_nat ((λ x, x = p)) n)

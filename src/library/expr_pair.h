...
inline bool is_equal(expr_pair const & p1, expr_pair const & p2) {
    return is_equal(p1.first, p2.first) && is_equal(p1.second, p2.second);
}

/** \brief Functional object for comparing expression pairs. */
struct expr_pair_eq {
    bool operator()(expr_pair const & p1, expr_pair const & p2) const { return is_equal(p1, p2); }
};
...
inline bool is_lt(expr_pair const & p1, expr_pair const & p2, bool use_hash) {
    return is_lt(p1.first, p2.first, use_hash) || (is_equal(p1.first, p2.first) && is_lt(p1.second, p2.second, use_hash));
}
struct expr_pair_quick_cmp {
    int operator()(expr_pair const & p1, expr_pair const & p2) const {
        return is_lt(p1, p2, true) ? -1 : (is_equal(p1, p2) ? 0 : 1);
    }
};
}
